...
from sam3d.training.network_training.Trainer_lung import Trainer_lung
...
class sam3d_trainer_lung(Trainer_lung):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.plans['plans_per_stage'][0]['patch_size'] = np.array([34, 192, 192])
            # self.crop_size = np.array([32, 192, 192])
            self.crop_size = np.array([34, 192, 192])

            # self.plans['plans_per_stage'][self.stage]['pool_op_kernel_sizes'] = [[1, 4, 4], [2, 2, 2], [2, 2, 2]]
            self.plans['plans_per_stage'][self.stage]['pool_op_kernel_sizes'] = [[1, 2, 2], [1, 2, 2], [1, 2, 2]]
